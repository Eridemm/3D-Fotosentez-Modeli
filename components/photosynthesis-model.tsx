...
export default function PhotosynthesisModel({
  ...
  const getOpacity = (processId: string) => {
    ...
    if (
      ...
      (processId === "calvin-cycle" || processId === "light-independent")
    ) {
      return 1
    }
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Light-dependent reactions - Added process highlighting */}
      ...
      {/* Chlorophyll and excited electrons - Added process highlighting */}
      ...
      {/* Electron transport chain - Added process highlighting */}
      ...
      {/* ATP Synthase - Added process highlighting */}
      ...
      {/* Water splitting (Photolysis) - Added process highlighting */}
      ...
      {/* Calvin Cycle (Light-independent reactions) - Enhanced visibility */}
      <group position={[0, -3.5, 0]} scale={getScale("calvin-cycle")}>
        <group ref={calvinRef}>
          ...
          <mesh renderOrder={24}>
            <torusGeometry args={[2.8, 0.18, 8, 20]} />
            <meshBasicMaterial color="#a855f7" transparent opacity={0.8 * getOpacity("calvin-cycle")} />
          </mesh>
          ...
          {Array.from({ length: 12 }, (_, i) => (
            ...
            >
              ...
              <meshBasicMaterial color="#c084fc" transparent opacity={0.7 * getOpacity("calvin-cycle")} />
            </mesh>
          ))}
        </group>

        <group position={[0, -2.5, 0]} renderOrder={29}>
          <mesh>
            <boxGeometry args={[1.4, 1.2, 0.4]} />
            <meshLambertMaterial color="#fbbf24" transparent opacity={getOpacity("calvin-cycle")} />
          </mesh>
          {/* Hexagon shape for glucose - made larger */}
          <mesh position={[0, 0, 0.25]} renderOrder={30}>
        ...
        {Array.from({ length: 6 }, (_, i) => (
          <group
            key={`energy-${i}`}
            position={[Math.cos((i * Math.PI) / 3) * 2.2, Math.sin((i * Math.PI) / 3) * 2.2, 0]}
            renderOrder={31}
          >
            <mesh>
              <sphereGeometry args={[0.12]} />
              <meshBasicMaterial
                ...
                transparent
                opacity={getOpacity("calvin-cycle")}
              />
            </mesh>
            {showLabels && i < 2 && (
              <Html position={[0, -0.5, 0]} center>
                <div className="text-xs font-bold text-purple-800">{i === 0 ? "ATP" : "NADPH"}</div>
              </Html>
            )}
          </group>
        ))}
      ...
      {/* Energy flow arrows - Added process highlighting */}
      <group>
